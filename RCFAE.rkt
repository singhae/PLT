...
(define-type RCFAEDS
    ...
    [mul (lhs RCFAEDS?) (rhs RCFAEDS?)] ; * operator
    ;[with (name symbol?) (named-expr RLFAE?) (body RLFAE?)]
    [id         (name symbol?)]
    [fun      (param symbol?) (body RCFAEDS?)]
    [app     (ftn RCFAEDS?) (arg RCFAEDS?)]
    [ifexp (test-expr RCFAEDS?) (then-expr RCFAEDS?)(else-expr RCFAEDS?)];if exp
    [orop (lhs RCFAEDS?) (rhs RCFAEDS?)] ;or operator
    [eqop (lhs RCFAEDS?) (rhs RCFAEDS?)]) ;equal operator
...
(define (is-recursion sexp)
  (match sexp
    [(list 'with (list i a) (list f b)) (if (check-id i a) true false)]  ;and (is-recursion a)  (app (fun i (parse e)) (app (parse f) (parse a)))]
  ;[(list 'fun (list p) b) true]
    [else                             false ]))
...
;[contract]a-recursion : list(sexp) -> list(sexp)                                                                       
;[purpose]: to make a recursion without rec
(define (a-recursion sexp)
  (match sexp
    [(list 'with (list i (list 'fun (list n) e)) (list i num))   (list 'with (list i (list 'with (list 'facX (list 'fun '(facY)(list 'with (list i '(facY facY))
                                                                    (list 'fun (list n) e))))
                          '(facX facX)))
      (list i num))]))
(a-recursion '{with {count {fun {n} {if {= n 0} 0 {+ 1 {count {- n 1}}}}}}
                      {count 8}})
;;[test1] desugar func
        ...
        [app   (f a)        (local [(define f-val (strict (interp f ds)))     ;ftn-v --> f-val , + strict()                     
                                             ...
                                             (interp (closureV-body f-val)
                                                          (aSub (closureV-param f-val)
                                                                      a-val
                                                                      (closureV-ds f-val))))]
        [ifexp (test-expr then-expr else-expr)    (if (true (interp test-expr ds) ) (interp then-expr ds) (interp else-expr ds))]))
...
;[test] run: sexp -> numV
(define (run sexp)
         (interp (parse(a-recursion sexp)) (mtSub)))
;[test1]
